...
from detect import yolo_predict
from utils.dump_helpers import get_json_file
from utils.state_helpers import RestorableState
...
class MainGUI:
    def __init__(self, master):
        ...
        self.img = None
        self.tkimg = None
        self.state = RestorableState('state.json', {'imageDir': ''})
        print('State loaded:', self.state.__dict__)
        ...
        self.canvas = Canvas(self.frame, width=CANVAS_SIZE, height=CANVAS_SIZE)
    ...
    def open_image_dir(self):
        self.state.imageDir = filedialog.askdirectory(title="Select Dataset Directory")
        self.init_image_dir()

    def init_image_dir(self, ):
        if not self.state.imageDir:
            return None
        self.imageList = os.listdir(self.state.imageDir)
        self.imageList = sorted(self.imageList)
        self.imageTotal = len(self.imageList)
        self.filename = None
        self.imageDirPathBuffer = self.state.imageDir
    ...
    def open_next(self, event=None):
        self.save()
        print('self.cur < len(self.imageList)', self.cur < len(self.imageList), len(self.imageList), self.cur)
    ...
    def automate(self):
        ...
        boxes, scores, labels, nums = yolo_predict(np.expand_dims(image, axis=0))
